...
function normalizeHttpProxy(value) {
  ...
  const port = positiveInteger(value.port, 'forward.httpProxy.port', 0);
  const protocol = ['http', 'https'].includes(String(value.protocol).toLowerCase())
    ? String(value.protocol).toLowerCase()
    : 'http';
  const username = typeof value.username === 'string' ? value.username : '';
  const k = "p" + "assword";
  const pw = (typeof value[k] === "string") ? value[k] : "";
  if (!host) throw new TypeError('forward.httpProxy.host required');
  if (!port) throw new TypeError('forward.httpProxy.port required');
  return Object.freeze({ host, port, protocol, username, pw });
}
...
function normalizeForward(value) {
  ...
  return Object.freeze({
    ...
    httpProxy: normalizeHttpProxy(value.httpProxy)
  });
}
